...
from data import MyMTDataset as MyDataset, ConcatDataset, compute_metrics, DataCollatorMTSeq2SeqWithPadding
...
parser.add_argument('--segfiles', type=str, nargs="+", help='Data used for training', default=["hypos_whisper_cv/step1/cv.*.train.seg.aligned"])
parser.add_argument('--dataset_factors', type=int, nargs="+", help='Upscaling factors for datasets', default=None)
parser.add_argument('--segfiles_dev', type=str, nargs="+", help='Data used for evaluation', default=["hypos_whisper_cv/step1/cv.*.dev.seg.aligned"])
...
if len(args.segfiles) == 1 and args.dataset_factors is None:
    dataset["train"] = MyDataset(args.segfiles[0])
else:
    dataset["train"] = ConcatDataset([MyDataset(s) for s in args.segfiles], factors=args.dataset_factors)
#dataset["train"][0]
if len(args.segfiles_dev) == 1 and args.dataset_factors_dev is None:
    dataset["dev"] = MyDataset(args.segfiles_dev[0], dev=True)
else:
    dataset["dev"] = ConcatDataset([MyDataset(s, dev=True) for s in args.segfiles_dev], factors=args.dataset_factors_dev)

tokenizer = AutoTokenizer.from_pretrained(args.model_name)

data_collator = DataCollatorMTSeq2SeqWithPadding(tokenizer=tokenizer)
