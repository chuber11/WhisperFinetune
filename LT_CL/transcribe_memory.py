...
for segfile in tqdm(glob(f"data_to_process_{user}/*.seg.aligned")):
    ...
    for line in open(segfile):
        ...
        res = requests.post(server, files={"pcm_s16le":wav_, "memory":json.dumps(memory_words), "prefix":"", "user":user})
        print(res.text)
        hypo = res.json()["hypo"]
    ...
    if len(all_s) > 0:
        for s,h,m in zip(all_s, all_h, all_m):
            split = "train"
            if all(word in recognized_words["train"] for word in m) and not all(word in recognized_words["dev"] for word in m):
                split = "dev"
            open(outfiles[("seg.aligned",split)],"a").write(s)
            open(outfiles[("hypo",split)],"a").write(h)
            open(outfiles[("memory",split)],"a").write("|".join(m))
            split2 = "dev" if split=="train" else "train" # Create files if not yet done
            open(outfiles[("seg.aligned",split2)],"a")
            open(outfiles[("hypo",split2)],"a")
            open(outfiles[("memory",split2)],"a")
            for word in m:
